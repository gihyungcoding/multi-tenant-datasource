...
- **Master/Slave 분리**: `@Transactional(readOnly = true)` 읽기 트랜잭션은 slave DB 로, 쓰기 트랜잭션은 master DB 로 자동 라우팅.([ADR 009](docs/adr/009-master-slave-datasource-routing.md) 참고).
...
| [ADR 007](docs/adr/007-atomic-datasource-registration.md) | DataSource 등록의 원자적 처리 — registerAndSnapshot + synchronized |
| [ADR 008](docs/adr/008-dual-entity-manager-factory.md) | Dual EntityManagerFactory — 마스터 DB와 테넌트 DB의 JPA 분리 |
| [ADR 009](docs/adr/009-master-slave-datasource-routing.md) | Master/Slave DataSource 라우팅 — 복합 키 + TransactionSynchronizationManager |
